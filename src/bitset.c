...
static size_t get_byte(size_t bit) {
	return bit >> 3;
}

static size_t byte_count(size_t bit) {
	return get_byte(bit) + 1;
}

static uint8_t get_bit_offset(size_t bit) {
	return 7 - (bit % 8);
}
...
int bitset_clone(const struct bitset *src, struct bitset *dest) {
	if (src == NULL || dest == NULL) return EXIT_FAILURE;

	if (bitset_init(dest, src->max)) return EXIT_FAILURE;
...
int bitset_eql(const struct bitset *a, const struct bitset *b) {
	for (size_t i = 0; i < (a->max > b->max ? a->max : b->max); i++) {
		if (!(bitset_has(a, i) == bitset_has(b, i))) return 0;
	}
	return 1;
}
...
int bitset_intersects(const struct bitset *a, const struct bitset *b) {
	for (size_t i = 0; bitset_next(a, &i); i++) {
		if (bitset_has(b, i)) return 1;
	}

	return 0;
}

int bitset_union(struct bitset *result, const struct bitset *a, const struct
		bitset *b) {
	const struct bitset *largest = a->max > b->max ? a : b;

	if (bitset_clone(largest, result)) return EXIT_FAILURE;
...
int bitset_difference(struct bitset *result, const struct bitset *a, const
		struct bitset *b) {
	const struct bitset *largest = a->max > b->max ? a : b;
